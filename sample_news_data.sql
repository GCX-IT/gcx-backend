-- Sample News Data for GCX News Ticker
-- Run this SQL script in your MySQL database to add test news items

-- First, let's insert some news categories
INSERT INTO news_categories (name, slug, created_at, updated_at) VALUES
...
-- Get the category IDs (adjust if your IDs are different)
SET @market_category_id = (SELECT id FROM news_categories WHERE slug = 'market-updates');
SET @announcement_category_id = (SELECT id FROM news_categories WHERE slug = 'announcements');
SET @partnership_category_id = (SELECT id FROM news_categories WHERE slug = 'partnerships');
SET @event_category_id = (SELECT id FROM news_categories WHERE slug = 'events');
